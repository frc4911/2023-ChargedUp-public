...
public abstract class TrajectoryFollower<DriveSignalType> {
    ...
    public final void follow(TrajectoryIterator<TimedState<Pose2dWithCurvature>> trajectory) {
        synchronized (trajectoryLock) {
     ...
     */
    public final Optional<TrajectoryIterator<TimedState<Pose2dWithCurvature>>> getCurrentTrajectory() {
        synchronized (trajectoryLock) {
