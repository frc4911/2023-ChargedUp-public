...
public class SwerveModuleState implements Comparable<SwerveModuleState> {
    ...
    @Override
    public int hashCode() {
        return Objects.hash(speedInMetersPerSecond);
    }
    ...
    public int compareTo(SwerveModuleState other) {
        return Double.compare(this.speedInMetersPerSecond, other.speedInMetersPerSecond);
    }

    @Override
    public String toString() {
        return String.format("SwerveModuleState(Speed: %.2f m/s, Angle: %s)", speedInMetersPerSecond,
                angle);
    }
    ...
    public static SwerveModuleState optimize(
        ...
        if (Math.abs(delta.getDegrees()) > 90.0) {
            return new SwerveModuleState(
                    -desiredState.speedInMetersPerSecond,
                    desiredState.angle.rotateBy(Rotation2d.fromDegrees(180.0)));
        } else {
            return new SwerveModuleState(desiredState.speedInMetersPerSecond, desiredState.angle);
        }
    }
}
